...
/// Since MQL5 doesn't allow DLLs to call its functions, our approach here is to inquire the DLL if there is an MQL function to be called
/// and let the MQL code do the call.
...
void init_rust_to_mql_method_calling_interface() {
   // pre-allocates the communications buffer
   StringReserve(calling_buffer, 1024);
}
